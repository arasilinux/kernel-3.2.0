...
static int tlc59108_probe(struct i2c_client *c, const struct i2c_device_id *id)
{
	 ...
	 */
	//if (cpu_is_am335x()) {    //Commented by tamilarasi on 21/01/2013
		/* Set LED4 to always on in LEDOUT1 Register*/
	//	ret = i2c_smbus_write_byte_data(data->client, TLC59108_LEDOUT1, 0x01); //commented by tamilarasi on 21/01/1013
		if(ret < 0)
